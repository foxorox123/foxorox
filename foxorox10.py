...
# === Treeview z sortowaniem i przewijaniem ===
...
tree = ttk.Treeview(tree_frame, columns=("Company", "Gap %", "Candle Color", "Candle %"), show="headings", height=10, yscrollcommand=scrollbar.set)
scrollbar.config(command=tree.yview)

tree.heading("Company", text="Company", command=lambda: sort_treeview(tree, "Company", False, is_percentage=False))
tree.heading("Gap %", text="Gap %", command=lambda: sort_treeview(tree, "Gap %", False, is_percentage=True))
tree.heading("Candle Color", text="Candle Color", command=lambda: sort_treeview(tree, "Candle Color", False, is_percentage=False))
...
def sort_treeview(tree, col, reverse, is_percentage=False):
    ...
    if is_percentage:
        def parse(x): return float(x[0].replace('%', '').replace('No data', '-1')) if x[0] != "-" else -1
        data.sort(key=parse, reverse=reverse)
    else:
        data.sort(key=lambda x: x[0], reverse=reverse)
    for index, (_, k) in enumerate(data):
        tree.move(k, '', index)
    tree.heading(col, command=lambda: sort_treeview(tree, col, not reverse, is_percentage))
