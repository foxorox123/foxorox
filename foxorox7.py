import pandas as pd
import tkinter as tk
from tkinter import ttk
from catboost import CatBoostClassifier
from datetime import datetime
import numpy as np
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
from matplotlib.figure import Figure
from matplotlib import dates as mdates
from matplotlib.patches import Rectangle
from PIL import Image, ImageTk
import yfinance as yf
import time

# 100 największych spółek S&P 500 i ich symbole
sp500_top10 = {
    'Apple': 'AAPL',
    'Microsoft': 'MSFT',
    'Alphabet (GOOGL)': 'GOOGL',
    'Amazon': 'AMZN',
    'NVIDIA': 'NVDA',
    'Meta Platforms': 'META',
    'Berkshire Hathaway': 'BRK-B',
    'UnitedHealth Group': 'UNH',
    'Eli Lilly': 'LLY',
    'Johnson & Johnson': 'JNJ',
    'ExxonMobil': 'XOM',
    'JPMorgan Chase': 'JPM',
    'Visa': 'V',
    'Procter & Gamble': 'PG',
    'Broadcom': 'AVGO',
    'Mastercard': 'MA',
    'Home Depot': 'HD',
    'Chevron': 'CVX',
    'AbbVie': 'ABBV',
    'Merck': 'MRK',
    'Costco': 'COST',
    'PepsiCo': 'PEP',
    'Pfizer': 'PFE',
    'Coca-Cola': 'KO',
    'Walmart': 'WMT',
    'Thermo Fisher Scientific': 'TMO',
    'Oracle': 'ORCL',
    'Walt Disney': 'DIS',
    'Cisco Systems': 'CSCO',
    'Comcast': 'CMCSA',
    'McDonald\'s': 'MCD',
    'Adobe': 'ADBE',
    'Bank of America': 'BAC',
    'Netflix': 'NFLX',
    'Salesforce': 'CRM',
    'Intel': 'INTC',
    'Texas Instruments': 'TXN',
    'Qualcomm': 'QCOM',
    'Accenture': 'ACN',
    'Amgen': 'AMGN',
    'IBM': 'IBM',
    'Philip Morris': 'PM',
    'Union Pacific': 'UNP',
    'NextEra Energy': 'NEE',
    'Raytheon Technologies': 'RTX',
    'Medtronic': 'MDT',
    'Honeywell': 'HON',
    'Nike': 'NKE',
    'Linde': 'LIN',
    'Lockheed Martin': 'LMT',
    'Danaher': 'DHR',
    'Bristol Myers Squibb': 'BMY',
    'Booking Holdings': 'BKNG',
    'United Parcel Service': 'UPS',
    'General Electric': 'GE',
    'Starbucks': 'SBUX',
    'Intuit': 'INTU',
    'American Express': 'AXP',
    'Charter Communications': 'CHTR',
    'S&P Global': 'SPGI',
    'CVS Health': 'CVS',
    'Marsh & McLennan': 'MMC',
    'ServiceNow': 'NOW',
    'Advanced Micro Devices': 'AMD',
    'ConocoPhillips': 'COP',
    'Caterpillar': 'CAT',
    'Mondelez': 'MDLZ',
    'Chubb': 'CB',
    'Cigna': 'CI',
    'General Motors': 'GM',
    'Goldman Sachs': 'GS',
    '3M': 'MMM',
    'Estee Lauder': 'EL',
    'Biogen': 'BIIB',
    'T-Mobile US': 'TMUS',
    'Ford Motor': 'F',
    'KLA Corporation': 'KLAC',
    'Illinois Tool Works': 'ITW',
    'American Tower': 'AMT',
    'CME Group': 'CME',
    'Colgate-Palmolive': 'CL',
    'Truist Financial': 'TFC',
    'Lam Research': 'LRCX',
    'Anthem': 'ANTM',
    'Dominion Energy': 'D',
    'Southern Company': 'SO',
    'PNC Financial': 'PNC',
    'Crown Castle': 'CCI',
    'Moody\'s': 'MCO',
    'Capital One': 'COF',
    'Northrop Grumman': 'NOC',
    'PayPal': 'PYPL',
    'Eaton': 'ETN',
    'Public Storage': 'PSA',
    'Corning': 'GLW',
    'Duke Energy': 'DUK',
    'Autodesk': 'ADSK',
    'Dow Inc.': 'DOW',
    'HCA Healthcare': 'HCA',
    'Nucor': 'NUE',
    'Freeport-McMoRan': 'FCX',
    'MetLife': 'MET',
    'General Dynamics': 'GD'
}

def fetch_data(symbol):
    for attempt in range(3):
        try:
            ticker = yf.Ticker(symbol)
            df = ticker.history(period="300d")
            if df.empty:
                print(f"No data for {symbol}")
                return pd.DataFrame()
            df.reset_index(inplace=True)
            df = df[['Date', 'Open', 'High', 'Low', 'Close', 'Volume']]
            return df
        except KeyboardInterrupt:
            print("Fetch data interrupted by user.")
            return pd.DataFrame()
        except Exception as e:
            print(f"Failed to get ticker '{symbol}', attempt {attempt+1}: {e}")
            time.sleep(2)
    return pd.DataFrame()

# reszta Twojego kodu dokładnie bez zmian:

def train_gap_model(df):
    df = df.copy()
    df['Gap'] = (df['Open'] - df['Close'].shift(1)) / df['Close'].shift(1)
    df['GapAboveThreshold'] = (df['Gap'].abs() > threshold.get() / 100).astype(int)
    df = df.dropna()
    X = df[['Open', 'High', 'Low', 'Close', 'Volume']]
    y = df['GapAboveThreshold']
    if len(y.unique()) == 1:
        raise ValueError("Only one unique value in target")
    model = CatBoostClassifier(
        iterations=iterations.get(),
        depth=depth.get(),
        learning_rate=learning_rate.get(),
        verbose=False
    )
    model.fit(X, y)
    return model

def train_candle_model(df):
    df = df.copy()
    df['NextCandleWhite'] = (df['Close'].shift(-1) > df['Open'].shift(-1)).astype(int)
    df = df.dropna()
    X = df[['Open', 'High', 'Low', 'Close', 'Volume']]
    y = df['NextCandleWhite']
    if len(y.unique()) == 1:
        raise ValueError("Only one unique value in target")
    model = CatBoostClassifier(
        iterations=iterations.get(),
        depth=depth.get(),
        learning_rate=learning_rate.get(),
        verbose=False
    )
    model.fit(X, y)
    return model

def plot_candlestick(ax, data):
    width = 0.6
    colors_up = 'green'
    colors_down = 'red'
    for idx, row in data.iterrows():
        date_num = mdates.date2num(row['Date'])
        open_, high, low, close = row['Open'], row['High'], row['Low'], row['Close']
        color = colors_up if close >= open_ else colors_down
        rect = Rectangle((date_num - width/2, min(open_, close)), width, abs(close - open_), color=color)
        ax.add_patch(rect)
        ax.plot([date_num, date_num], [low, high], color=color, linewidth=1)

def plot_bar_chart(ax, data):
    width = 0.4
    for idx, row in data.iterrows():
        date_num = mdates.date2num(row['Date'])
        open_, high, low, close = row['Open'], row['High'], row['Low'], row['Close']

        # linia od low do high
        ax.plot([date_num, date_num], [low, high], color='black', linewidth=1)

        # poziome kreski open i close
        ax.plot([date_num - width, date_num], [open_, open_], color='green', linewidth=3)   # open po lewej
        ax.plot([date_num, date_num + width], [close, close], color='red', linewidth=3)     # close po prawej

def show_plot(df, title):
    for widget in chart_frame.winfo_children():
        widget.destroy()

    fig = Figure(figsize=(7, 6), dpi=100)
    ax1 = fig.add_subplot(211)
    ax2 = fig.add_subplot(212, sharex=ax1)

    data_tail = df.tail(50).copy()
    data_tail['Volume'] = data_tail['Volume'] / 1_000_000  # mln shares

    if plot_type.get() == "Line":
        ax1.plot(data_tail['Date'], data_tail['Close'], label="Close", color='blue')
    elif plot_type.get() == "Candlestick":
        plot_candlestick(ax1, data_tail)
    elif plot_type.get() == "Bar":
        plot_bar_chart(ax1, data_tail)

    ax1.set_title(title)
    ax1.set_ylabel("Price")
    ax1.grid(True)
    ax1.xaxis.set_major_formatter(mdates.DateFormatter('%d-%m'))

    ax2.bar(data_tail['Date'], data_tail['Volume'], color='gray', width=0.6)
    ax2.set_ylabel("Volume (mln shares)")
    ax2.grid(True)

    fig.autofmt_xdate(rotation=45)
    fig.subplots_adjust(hspace=0.1)

    canvas = FigureCanvasTkAgg(fig, master=chart_frame)
    canvas.draw()
    canvas.get_tk_widget().pack(fill='both', expand=True)

def update_prediction():
    try:
        symbol = sp500_top10[company_var.get()]
        df = fetch_data(symbol)
        if df.empty:
            result_var.set("No data available")
            return

        gap_model = train_gap_model(df)
        candle_model = train_candle_model(df)
        latest = df[['Open', 'High', 'Low', 'Close', 'Volume']].iloc[[-1]]

        gap_proba = gap_model.predict_proba(latest)[0][1]
        candle_proba = candle_model.predict_proba(latest)[0][1]
        candle_color = "white (bullish)" if candle_proba > 0.5 else "black (bearish)"

        result_var.set(
            f"{company_var.get()}:\n"
            f" - Chance for gap > {threshold.get()}%: {gap_proba*100:.2f}%\n"
            f" - Next candle predicted: {candle_color}\n"
            f" - Probability: {candle_proba*100:.2f}%"
        )
        show_plot(df, f"Chart: {company_var.get()}")
    except Exception as e:
        result_var.set("❌ Analysis error.")
        with open("errors.log", "a") as f:
            f.write(f"[{datetime.now()}] {type(e).__name__}: {str(e)}\n")

def calculate_all():
    for row in tree.get_children():
        tree.delete(row)

    results = []

    for company, symbol in sp500_top10.items():
        df = fetch_data(symbol)
        if df.empty:
            results.append((company, None, None, None, df))
            continue
        try:
            gap_model = train_gap_model(df)
            candle_model = train_candle_model(df)
        except Exception:
            results.append((company, None, None, None, df))
            continue

        latest = df[['Open', 'High', 'Low', 'Close', 'Volume']].iloc[[-1]]

        gap_proba = gap_model.predict_proba(latest)[0][1]
        candle_proba = candle_model.predict_proba(latest)[0][1]
        candle_color = "white" if candle_proba > 0.5 else "black"

        results.append((company, gap_proba*100, candle_color, candle_proba*100, df))

    results.sort(key=lambda x: (x[3] is None, -x[3] if x[3] is not None else 0))

    for company, gap_pct, candle_col, candle_pct, df in results:
        if df.empty:
            tree.insert("", "end", values=(company, "No data available", "-", "-"))
        else:
            tree.insert("", "end", values=(company, f"{gap_pct:.2f}%", candle_col, f"{candle_pct:.2f}%"))

def on_tree_select(event):
    selected = tree.selection()
    if selected:
        values = tree.item(selected[0], "values")
        company = values[0]
        symbol = sp500_top10.get(company)
        if symbol:
            df = fetch_data(symbol)
            if df.empty:
                result_var.set("No data available for chart")
                for widget in chart_frame.winfo_children():
                    widget.destroy()
                return
            show_plot(df, f"Chart: {company}")

root = tk.Tk()
root.title("Stock Gap and Candle Prediction")
root.geometry("900x700")

frame_top = tk.Frame(root)
frame_top.pack(fill="x", padx=10, pady=5)

company_var = tk.StringVar(value=list(sp500_top10.keys())[0])
company_menu = ttk.Combobox(frame_top, values=list(sp500_top10.keys()), textvariable=company_var, state="readonly")
company_menu.pack(side="left")

threshold = tk.DoubleVar(value=1.0)
iterations = tk.IntVar(value=100)
depth = tk.IntVar(value=4)
learning_rate = tk.DoubleVar(value=0.1)
plot_type = tk.StringVar(value="Candlestick")

tk.Label(frame_top, text="Gap Threshold %").pack(side="left", padx=5)
tk.Entry(frame_top, textvariable=threshold, width=5).pack(side="left")
tk.Label(frame_top, text="Iterations").pack(side="left", padx=5)
tk.Entry(frame_top, textvariable=iterations, width=5).pack(side="left")
tk.Label(frame_top, text="Depth").pack(side="left", padx=5)
tk.Entry(frame_top, textvariable=depth, width=5).pack(side="left")
tk.Label(frame_top, text="Learning Rate").pack(side="left", padx=5)
tk.Entry(frame_top, textvariable=learning_rate, width=5).pack(side="left")

plot_menu = ttk.Combobox(frame_top, values=["Line", "Candlestick", "Bar"], textvariable=plot_type, state="readonly", width=10)
plot_menu.pack(side="left", padx=10)

btn_predict = tk.Button(frame_top, text="Predict for selected", command=update_prediction)
btn_predict.pack(side="left", padx=10)

btn_all = tk.Button(frame_top, text="Calculate All", command=calculate_all)
btn_all.pack(side="left")

result_var = tk.StringVar()
result_label = tk.Label(root, textvariable=result_var, font=("Arial", 12), justify="left")
result_label.pack(fill="x", padx=10, pady=5)

chart_frame = tk.Frame(root)
chart_frame.pack(fill="both", expand=True, padx=10, pady=5)

tree = ttk.Treeview(root, columns=("Company", "Gap %", "Candle Color", "Candle %"), show="headings")
tree.heading("Company", text="Company")
tree.heading("Gap %", text="Gap %")
tree.heading("Candle Color", text="Candle Color")
tree.heading("Candle %", text="Candle %")
tree.pack(fill="both", expand=True, padx=10, pady=5)
tree.bind("<<TreeviewSelect>>", on_tree_select)

root.mainloop()
