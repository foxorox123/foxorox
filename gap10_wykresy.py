...
# ✅ Tickery Yahoo Finance dla spółek WIG20
wig20_symbols = {
    'ALLEGRO': 'ALE.WA',
    'ALIOR': 'ALR.WA',
    'BUDIMEX': 'BDX.WA',
...
def fetch_data(symbol):
    try:
        df = yf.download(symbol, period="1y", interval="1d", auto_adjust=False)
        if df.empty:
            raise Exception("Brak danych z Yahoo Finance")
        ...
        print(f"Błąd pobierania danych z Yahoo Finance dla {symbol}: {e}")
        return pd.DataFrame()
...
def show_plot(df, title):
    ...
    if data_tail.empty or 'Volume' not in data_tail.columns:
        return

    try:
        volume_raw = data_tail['Volume']
        if isinstance(volume_raw, pd.DataFrame):
            volume_raw = volume_raw.iloc[:, 0]
        volume_series = pd.to_numeric(volume_raw, errors='coerce')
        data_tail = data_tail[volume_series.notnull()]
        volume_series = volume_series.apply(lambda x: float(x[0]) if isinstance(x, (list, np.ndarray)) else float(x))
        data_tail['Volume'] = volume_series / 1_000_000
    except Exception as e:
        print(f"Błąd przetwarzania Volume: {e}")
        return
    ...
    ax2.set_ylabel("Wolumen (mln akcji)")
    ax2.grid(True)
...
def calculate_all():
    ...
    for company, gap_pct, candle_col, candle_pct, df in results:
        if candle_pct is not None and not df.empty:
