...
def update_data_hourly(symbol):
    symbol_yahoo = fix_symbol_yahoo(symbol)
    file_path = os.path.join(DATA_DIR, f"{symbol_yahoo}.csv")

    if not os.path.exists(file_path):
        print(f"⚠️ File {file_path} does not exist, fetching full data.")
    ...
    # Zakładam, że dane są codzienne, więc uaktualniamy od dnia po ostatnim
    start_date = last_date + timedelta(days=1)
    today = datetime.now().date()

    if start_date.date() > today:
        print(f"✅ Dane dla {symbol_yahoo} są już aktualne.")
        return df

    print(f"🔄 Aktualizacja danych dla {symbol_yahoo} od {start_date.date()}")

    new_data = yf.Ticker(symbol_yahoo).history(start=start_date.strftime('%Y-%m-%d'))
    if new_data.empty:
...
# Przykładowa funkcja która co godzinę odświeża dane
def run_hourly_update(symbol):
    while True:
        update_data_hourly(symbol)
        print(f"⏰ Czekam godzinę do kolejnej aktualizacji...")
...
def export_to_excel():
    ...
    if not rows:
        messagebox.showwarning("Export", "No data to export. Run 'Calculate All' first.")
        return

    df_export = pd.DataFrame(rows, columns=["Company", "Gap %", "Candle Color", "Candle %"])
