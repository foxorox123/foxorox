...
def show_plot(df, title):
    ...
    if plot_type.get() == "Liniowy":
        # Wykres liniowy
        ax.plot(data_tail['Date'], data_tail['Close'], label="Zamknięcie", color='blue')
    else:
        # Wykres świecowy
        plot_candlestick(ax, data_tail)
    ...
    # Formatowanie osi X na dzień-miesiąc
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%d-%m'))
    fig.autofmt_xdate(rotation=45)

    # Dodaj radio buttony w prawym górnym rogu wykresu (umieszczone w tkinterze)
    # Tu zrobimy osobny frame nad wykresem, więc tu nic nie robimy.
...
plot_type = tk.StringVar(value="Liniowy")  # domyślnie wykres liniowy
...
# Ramka dla wyboru wykresu - umieszczona nad wykresem, po prawej stronie
...
radio_line = ttk.Radiobutton(plot_type_frame, text="Liniowy", variable=plot_type, value="Liniowy", command=on_plot_type_change)
radio_line.pack(side='left', padx=5)
radio_candle = ttk.Radiobutton(plot_type_frame, text="Świecowy", variable=plot_type, value="Świecowy", command=on_plot_type_change)
radio_candle.pack(side='left', padx=5)
