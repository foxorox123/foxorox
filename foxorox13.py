...
def fetch_data(symbol):
    # Użyj poprawionego symbolu
    symbol = fix_symbol_yahoo(symbol)

    # Jeśli dane już są w cache i nie trzeba dogrywać, zwróć je
    if symbol in company_data_cache:
        # W godzinach sesji pobieramy nowe dane i aktualizujemy cache
        if not is_market_open():
            return company_data_cache[symbol]
